...
BEGIN
	...
	OR amount_insert = 0 THEN
		RAISE EXCEPTION 'param_product_id and amount_insert cannot be null or zero';
	END IF;
	...
	IF (available_quantity + amount_insert) > 1000 THEN
		RAISE EXCEPTION 'The limit available amount per stock is 1000';
	END IF;
...
CALL commerce.increment_stock(10, 100);
SELECT stock_quantity FROM commerce.stock WHERE product_id = 10;